...
Always run this code chunk whenever you facing function packages problems.
...
The code chunk below is used to plot a scatterplot matrix of the relationship between the independent variables in *condo_resale* data.frame. Do not use the sf data.frame as it contains the geometry which will give a error.
...
The code chunk below using `lm()` to calibrate the multiple linear regression model. SELLING_PRICE is the dependent varibale and the rest are all independent variable.
...
The [**gtsummary**](https://www.danieldsjoberg.com/gtsummary/) package provides an elegant and flexible way to create publication-ready summary tables in R. allow us to generate very elegant report.
...
First part is globle, second part is local.

Feographic weighted perform a bit better than non geographically weighted.

Use aic to compare fix and adaptive. see which one is the most suitable one. which give you the smallest. plot the map for the best one.
...
tm-view(set.zoom.limites = c(11,14)) it can set limit for zoom in and zoom out.
...
```{r}
tmap_mode("view")
AREA_SQM_SE <- tm_shape(mpsz_svy21)+  tm_polygons(alpha = 0.1) +
tm_shape(condo_resale.sf.adaptive) +  
  tm_dots(col = "AREA_SQM_SE",
          border.col = "gray60",
          border.lwd = 1) +  tm_view(set.zoom.limits = c(11,14))
